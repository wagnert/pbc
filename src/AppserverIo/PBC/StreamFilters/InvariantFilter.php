...
/**
 * AppserverIo\PBC\StreamFilters\InvariantFilter
 *
 * NOTICE OF LICENSE
 *
...
class InvariantFilter extends AbstractFilter
{
     ...
     *
     * @param \AppserverIo\PBC\Entities\Lists\AttributeDefinitionList $attributeDefinitions Defined attributes
     *
     * @return string
     */
    ...
    private function generateSetCode($hasParents, $injected = false)
    {
        ...
        if ($hasParents) {
            ...
            $code .= 'if (property_exists($this, $name)) {' .
                ...
                '\AppserverIo\PBC\ContractContext::close();
                return false;
                } else {' .
    ...
    private function generateFunctionCode(TypedListList $assertionLists)
    {
        $code = 'protected function ' . PBC_CLASS_INVARIANT_NAME . '($callingMethod) {' .
            PBC_CONTRACT_CONTEXT . ' = \AppserverIo\PBC\ContractContext::open();if (' . PBC_CONTRACT_CONTEXT . ') {';
        ...
        $code .= '}\AppserverIo\PBC\ContractContext::close();}';
